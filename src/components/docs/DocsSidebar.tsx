...
export function DocsSidebar() {
  ...
  const links = [
    ...
    },
    {
      title: "kaisa AI",
      items: [
        { name: "Overview", href: "/docs/kaisa", icon: BrainIcon },
  ...
  return (
    <div className="w-64 flex-shrink-0 border-r border-white/10 bg-black h-[calc(100vh-80px)] sticky top-20 overflow-y-auto custom-scrollbar hidden lg:block">
      <div className="p-6">
        <div className="mb-8">
           <div className="flex items-center gap-2 text-brand-saffron font-bold text-xl mb-1">
             <Book className="w-5 h-5" />
             Docs
           </div>
           <p className="text-xs text-white/40">Version 2.0.4</p>
        </div>
                ...
                {section.items.map((item) => {
                  ...
                  return (
                    <li key={item.href}>
                      ...
                      >
                        <item.icon className={`w-4 h-4 ${isActive ? "text-brand-saffron" : "opacity-70"}`} />
                        {item.name}
                      </Link>
                    </li>
                  );
