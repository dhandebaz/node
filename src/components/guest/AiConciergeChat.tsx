    ...
    initialMessages: [
      {
        id: "welcome",
        role: "assistant",
        content: `Hi! I'm your AI Concierge for ${propertyName}. How can I help you today? You can ask me about Wi-Fi, property rules, or how to use the smart lock.`
      }
    ]
  });
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8 scroll-smooth scrollbar-none">
              {messages.map((m) => (
                <div key={m.id} className={cn("flex gap-4 items-start", m.role === "user" ? "flex-row-reverse" : "")}>
                  ...
                  >
                    <ReactMarkdown className="prose prose-invert prose-sm max-w-none prose-p:leading-relaxed prose-pre:bg-black/50 prose-pre:border prose-pre:border-white/5">
                      {m.content}
                    </ReactMarkdown>
                  </div>
                </div>
