...
import { SpaceAuditLog } from "@/types/space";
import { useState } from "react";
import { FileText, Clock, AlertTriangle, ToggleLeft, Activity, Settings } from "lucide-react";

export function SpaceAuditLogView({ logs }: { logs: SpaceAuditLog[] }) {
  const [showAll, setShowAll] = useState(false);
  const displayLogs = showAll ? logs : logs.slice(0, 5);

  const getIcon = (type: SpaceAuditLog["actionType"]) => {
    switch (type) {
      case "status_change": return <AlertTriangle className="w-4 h-4 text-red-500" />;
      case "plan_toggle": return <ToggleLeft className="w-4 h-4 text-blue-500" />;
      case "limit_change": return <Activity className="w-4 h-4 text-amber-500" />;
      case "dc_assignment": return <Settings className="w-4 h-4 text-purple-500" />;
                ...
                <div key={log.id} className="p-3 text-sm flex gap-3 hover:bg-zinc-900/30 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex gap-4 mt-1 text-xs font-mono text-zinc-600">
                           {log.targetServiceId && <span>SVC: {log.targetServiceId}</span>}
                           {log.targetUserId && <span>USR: {log.targetUserId}</span>}
                        </div>
                    </div>
                </div>
            ))
