  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        ...
        <p className="text-white/40 text-sm">Loading Kaisa's Growth Engine...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8 pb-20 md:pb-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <Button 
          onClick={handleScan} 
          disabled={scanning}
          className="bg-orange-500 hover:bg-orange-600 text-white font-bold px-6 rounded-full"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-zinc-900 border-white/5">
          <CardHeader className="pb-2">
            <CardDescription className="text-[10px] uppercase font-bold tracking-widest text-zinc-500">Gaps Detected</CardDescription>
            <CardTitle className="text-3xl font-bold text-white">{data.opportunities.length}</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="bg-zinc-900 border-white/5">
          <CardHeader className="pb-2">
            <CardDescription className="text-[10px] uppercase font-bold tracking-widest text-zinc-500">Active Campaigns</CardDescription>
            <CardTitle className="text-3xl font-bold text-white">{data.campaigns.length}</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="bg-zinc-900 border-white/5">
          <CardHeader className="pb-2">
            <CardDescription className="text-[10px] uppercase font-bold tracking-widest text-zinc-500">Revenue Generated</CardDescription>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {data.opportunities.length === 0 ? (
            <div className="bg-zinc-900/50 border border-white/5 rounded-2xl p-12 text-center space-y-4">
              ...
              <div>
                <h3 className="text-white font-bold">No opportunities found</h3>
                <p className="text-white/40 text-sm">Try scanning for availability gaps or check your integrations.</p>
              </div>
              <Button variant="outline" onClick={handleScan} className="border-white/10 text-white hover:bg-white/5">
                Refresh Calendar
              </Button>
            </div>
          ) : (
            <div className="space-y-4">
              {data.opportunities.map((o: any) => (
                <Card key={o.id} className="bg-zinc-900 border-white/5 hover:border-orange-500/30 transition-all overflow-hidden group">
                  <div className="flex flex-col md:flex-row">
                    <div className="p-6 flex-1 space-y-4">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="text-sm font-bold text-white">{o.guestName}</div>
                            <div className="text-[10px] text-zinc-500 uppercase tracking-widest">{o.listingTitle}</div>
                          </div>
                        </div>
                      ...
                      <div className="bg-black/20 p-4 rounded-xl border border-white/5 text-sm text-white/80 leading-relaxed italic">
                        "{o.message}"
                      </div>
                    ...
                    <div className="bg-white/5 md:w-48 p-4 flex md:flex-col justify-center gap-2 border-t md:border-t-0 md:border-l border-white/5">
                      ...
                      <Button 
                        variant="ghost" 
                        className="flex-1 text-zinc-500 hover:text-red-400 hover:bg-red-500/10 text-xs font-bold py-1"
                      >
                        Dismiss
                      </Button>
                    </div>
        ...
        <div className="space-y-6">
          ...
          <Card className="bg-zinc-900 border-white/5">
            <CardHeader className="pb-4 border-b border-white/5">
              <CardTitle className="text-sm text-white">Gap Filler</CardTitle>
              <CardDescription className="text-xs">Fills contiguous empty dates automatically</CardDescription>
            </CardHeader>
            <CardContent className="pt-4 space-y-4">
              <div className="flex justify-between items-center text-xs">
                <span className="text-zinc-500 uppercase font-bold tracking-tighter">Status</span>
                <span className="text-emerald-400 font-bold bg-emerald-500/10 px-2 py-0.5 rounded">Active</span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-zinc-500 uppercase font-bold tracking-tighter">Conversion Rate</span>
          ...
          <Card className="bg-zinc-900 border-white/5 opacity-50">
            <CardHeader className="pb-4 border-b border-white/5">
              <CardTitle className="text-sm text-white flex items-center gap-2">
                Review Boost
                <span className="text-[10px] bg-white/10 text-white/60 px-1.5 py-0.5 rounded uppercase tracking-tighter font-bold">Soon</span>
              </CardTitle>
              <CardDescription className="text-xs">Follows up with guests after checkout</CardDescription>
            </CardHeader>
            <CardContent className="pt-4">
              <Button disabled className="w-full bg-white/10 text-white/30 text-xs font-bold">
                Coming Soon
              </Button>
            </CardContent>
          </Card>
