...
export default function CalendarPage() {
  ...
  const statusDisplay = (status: string) => {
    ...
    if (status === "draft") {
      return { label: "draft", text: "text-foreground/50", dot: "bg-white/30" };
    }
    ...
    return { label: status, text: "text-foreground/60", dot: "bg-white/40" };
  };
  ...
  if (!capabilities.calendar) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center space-y-4">
        <div className="p-4 rounded-full bg-white/10 text-foreground/40">
          <CalendarIcon className="w-8 h-8" />
        </div>
        <div className="space-y-2">
          <h3 className="text-xl font-semibold text-foreground">Calendar Not Available</h3>
  ...
  return (
    <div className="space-y-4 pb-24 md:pb-0">
      {/* Header Controls */}
      <div className="flex flex-col gap-4 sticky top-0 bg-primary z-30 pt-2 pb-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-foreground uppercase tracking-tight">{labels.calendar}</h1>
          <div className="flex items-center gap-3">
            <button
              onClick={openPaymentModal}
              className="inline-flex items-center gap-2 text-xs font-semibold px-3 py-2 rounded-full border border-white/20 text-foreground/80 hover:border-white/50"
            >
              <Plus className="w-4 h-4" />
              Create {labels.booking.toLowerCase()}
            </button>
            <div className="flex bg-transparent p-1 rounded-lg border border-border">
            <button 
              onClick={() => setView('agenda')}
              className={cn(
                "p-2 rounded-md transition-all",
                view === 'agenda' ? "bg-white/10 text-foreground shadow-sm" : "text-foreground/40 hover:text-foreground/70"
              )}
            >
            ...
            <button 
              ...
              className={cn(
                "p-2 rounded-md transition-all",
                view === 'month' ? "bg-white/10 text-foreground shadow-sm" : "text-foreground/40 hover:text-foreground/70"
              )}
            >
        ...
        <div className="relative">
          <select 
            ...
            className="w-full appearance-none bg-transparent border border-border text-foreground rounded-xl px-4 py-3 pr-10 focus:outline-none focus:border-white/30 font-medium"
          >
            {listings.length > 1 && (
          ...
          <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 text-foreground/40 pointer-events-none w-5 h-5" />
        </div>
      </div>
      ...
      {view === 'agenda' ? (
        <div className="space-y-3">
          {isLoading && bookings.length === 0 ? (
            <div className="text-center py-10 text-foreground/40">Loading bookings...</div>
          ) : bookings.length === 0 ? (
            <div className="text-center py-10 text-foreground/40 dashboard-surface">
            ...
            bookings
              ...
              .map(booking => (
              <div key={booking.id} className="bg-transparent border border-border rounded-2xl p-4 active:scale-[0.98] transition-transform">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h3 className="font-bold text-foreground text-lg">{booking.guestName || labels.customer}</h3>
                  ...
                  <div className="text-right">
                    <div className="text-xs font-mono text-foreground/40 uppercase tracking-widest mb-1">{labels.checkIn}</div>
                    <div className="font-bold text-foreground">{format(parseISO(booking.startDate), isDoctor ? 'h:mm a' : 'MMM d')}</div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 py-3 border-t border-white/5 border-b mb-3">
                   <div className="flex-1">
                     ...
                     <div className="font-medium text-foreground">{format(parseISO(booking.startDate), isDoctor ? 'EEE, MMM d • h:mm a' : 'EEE, MMM d')}</div>
                   </div>
                   ...
                   <div className="flex-1">
                     ...
                     <div className="font-medium text-foreground">{format(parseISO(booking.endDate), isDoctor ? 'EEE, MMM d • h:mm a' : 'EEE, MMM d')}</div>
                   </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <a
                    ...
                    className="text-xs font-bold text-primary bg-white px-3 py-1.5 rounded-lg"
                  >
                    View listing
                  </a>
                </div>
        ...
        <div className="bg-transparent border border-border rounded-2xl overflow-hidden">
          <div className="p-4 flex items-center justify-between border-b border-border">
            <button onClick={prevMonth} className="p-2 hover:bg-white/5 rounded-full text-foreground/60">
              <ChevronLeft className="w-5 h-5" />
            </button>
            <span className="font-bold text-foreground">
              {format(currentDate, 'MMMM yyyy')}
            </span>
            <button onClick={nextMonth} className="p-2 hover:bg-white/5 rounded-full text-foreground/60">
              <ChevronRight className="w-5 h-5" />
            </button>
          </div>
          
          <div className="grid grid-cols-7 text-center border-b border-border bg-white/5">
            {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map(d => (
              <div key={d} className="py-2 text-[10px] font-bold text-foreground/40">{d}</div>
            ))}
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="bg-transparent border border-border rounded-2xl w-full max-w-2xl overflow-hidden">
            <div className="p-4 border-b border-border flex items-center justify-between">
              <div className="text-sm font-semibold text-foreground">Create {labels.booking.toLowerCase()} payment link</div>
              <button
            ...
            <div className="p-4 space-y-4 max-h-[70vh] overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-foreground/60 mb-1">{labels.listing}</label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-semibold text-foreground/60 mb-1">Amount</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs font-semibold text-foreground/60 mb-1">{labels.customer} name</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-semibold text-foreground/60 mb-1">{labels.customer} phone</label>
                  <input
                    type="tel"
                ...
                <div>
                  <label className="block text-xs font-semibold text-foreground/60 mb-1">{labels.customer} email</label>
                  <input
                    type="email"
                ...
                <div>
                  <label className="block text-xs font-semibold text-foreground/60 mb-1">{labels.checkIn}</label>
                  <input
                    type="date"
                ...
                <div>
                  <label className="block text-xs font-semibold text-foreground/60 mb-1">{labels.checkOut}</label>
                  <input
                    type="date"
              ...
              <div>
                <label className="block text-xs font-semibold text-foreground/60 mb-1">Notes (optional)</label>
                <textarea
              ...
              {paymentLink && (
                <div className="space-y-3 border border-border rounded-xl p-4 bg-white/5">
                  <div className="text-xs font-semibold text-foreground/70 uppercase tracking-wider">Payment link</div>
                  <div className="text-sm text-foreground break-all">{paymentLink}</div>
