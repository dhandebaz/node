...
export default function VerificationPage() {
  ...
  const checkHandle = async (val: string) => {
      ...
      // Mock check
      // In real app, call API to check uniqueness
    ...
    <div className="max-w-2xl mx-auto py-10 space-y-8">
      <div className="flex items-center justify-between mb-8">
        {[1, 2, 3, 4].map((s) => (
          <div key={s} className="flex items-center">
            ...
            {s < 4 && <div className={`w-12 h-1 mx-2 ${step > s ? "bg-purple-600" : "bg-zinc-800"}`} />}
          </div>
        ))}
      </div>
        ...
        <div className="bg-zinc-900 p-6 rounded-xl border border-zinc-800 space-y-6">
          ...
          {!extractedData ? (
            <div className="border-2 border-dashed border-zinc-700 rounded-xl p-8 text-center hover:border-purple-500 transition-colors">
              ...
              <p className="text-zinc-400 mb-4">Upload Aadhaar or Passport (Max 5MB)</p>
              <Input 
                type="file" 
            ...
            <div className="space-y-4">
              <div className="bg-green-900/20 border border-green-500/20 p-4 rounded-lg flex items-center gap-3">
                ...
                <span className="text-green-200">Document Processed Successfully</span>
              </div>
              ...
              <div className="grid gap-4 bg-zinc-950 p-4 rounded-lg">
                <div className="grid grid-cols-2 gap-4">
                   ...
                   <div>
                     ...
                     <div className="font-medium">{extractedData.document_number}</div>
                   </div>
                </div>
              </div>

              <Button onClick={handleKYCConfirm} disabled={loading} className="w-full">
      ...
      {step === 3 && (
        <div className="bg-zinc-900 p-6 rounded-xl border border-zinc-800 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-zinc-500">Draw your signature using mouse or touch.</p>
          </div>

          <Button onClick={handleAgreementSign} disabled={loading} className="w-full">
            {loading ? <Loader2 className="animate-spin" /> : "Sign & Complete Verification"}
          </Button>
        </div>
      )}
      ...
      {step === 4 && (
        <div className="bg-zinc-900 p-6 rounded-xl border border-zinc-800 space-y-6 text-center">
            ...
            <p className="text-zinc-400">Choose a unique username for your public profile.</p>
            
            <div className="flex items-center justify-center gap-2 max-w-sm mx-auto">
                <span className="text-zinc-500">nodebase.co/@</span>
                <Input 
                    value={handle}
                    onChange={(e) => checkHandle(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ''))}
            ...
            <Button onClick={handleUsernameSubmit} disabled={!handleAvailable || loading} className="w-full max-w-sm">
