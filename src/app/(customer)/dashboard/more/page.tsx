  ...
  return (
    <div className="space-y-6 pb-24 md:pb-0">
      ...
      <div className="flex items-center gap-4 p-4 dashboard-surface">
        <div className="w-16 h-16 rounded-full bg-primary border-2 border-border flex items-center justify-center text-xl font-bold text-foreground shadow-lg">
           {host?.name ? host.name.charAt(0) : 'U'}
        </div>
        <div>
          <h1 className="text-xl font-bold text-foreground tracking-tight">{host?.name || "User"}</h1>
          <p className="text-foreground/60 text-sm">{host?.email || "No email linked"}</p>
      ...
      <div className="space-y-6">
        {menuItems.map((section, idx) => (
          <div key={idx}>
            <h2 className="text-xs font-bold text-foreground/40 uppercase tracking-widest px-2 mb-2">
              {section.section}
            </h2>
              ...
              {section.items.map((item) => (
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="p-2 bg-white/5 rounded-lg text-foreground/70 group-hover:text-foreground group-hover:bg-white/10 transition-colors">
