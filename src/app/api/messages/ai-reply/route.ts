...
export async function POST(request: Request) {
  try {
    ...
    const aiBlockers = blockers.filter(
      (f) => f.category === "ai" || f.category === "payment",
    );
    ...
    const {
      ...
      limit: aiLimit,
      current: aiCurrent,
    } = await SubscriptionService.checkDailyLimit(tenantId, "ai_replies");
    if (!aiAllowed) {
      await logEvent({
        tenant_id: tenantId,
        actor_type: "ai",
    ...
    const { data: tenant } = await supabase
      .from("tenants")
      .select(
        "business_type, is_memory_enabled, is_branding_enabled, ai_settings",
      )
      .eq("id", tenantId)
      .single();
...
`;
    ...
    const actualCost = await PricingService.calculateCost(
      "ai_reply",
      Math.max(tokensUsed, MIN_ESTIMATED_TOKENS),
      tenantId,
    );
